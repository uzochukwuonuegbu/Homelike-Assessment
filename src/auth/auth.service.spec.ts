...
describe('AuthService', () => {
	...
	describe('Login', () => {
		...
		it('should throw if user credentials is invalid', async () => {
			const randomPassword = hashSync('randomPassword', salt);
		...
		it('should successfully log user in', async () => {
			const randomPassword = await hashSync('randomPassword', salt);
	...
	describe('Register', () => {
		it('should throw if user already exists', async () => {
			...
			try {
				...
				expect(bcrypt.hashSync).not.toHaveBeenCalledWith();
			} catch (error) {
